...
export const media_store = makeStore({});
export const ui_store = writable({
    modules_in_view: ["timeline", "media"],
    media_in_view: [],
    media_hovered: [],
...
export const media_store_filtered = derived([filter_toggles, media_store], ([$filter_toggles, $media_store]) => {
    // for each medium
    const filtered_media = Object.keys($media_store).filter((UAR) => {
        const medium = $media_store[UAR];
        // build an array with an item
        // for each on property, ie we want to see any video that has at least this property
        const some_properties_matches = Object.keys($filter_toggles)
            .some(filter => {
                const filter_value = $filter_toggles[filter];
